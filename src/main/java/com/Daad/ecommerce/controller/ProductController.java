...
public class ProductController {
    ...
    public ResponseEntity<Map<String, Object>> getAllProducts(
        ...
        try {
            ...
            // Only show approved or awaiting_approval unless explicitly requested
            if (!showAllStatuses) {
                filteredProducts = filteredProducts.stream()
                        .filter(product -> Arrays.asList("approved", "awaiting_approval")
                                .contains(product.getStatus() != null ? product.getStatus().toLowerCase() : ""))
                        .collect(Collectors.toList());
            }
            
            // Apply category filter
            if (category != null && !category.isEmpty()) {
                if (category.matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                    // If it's a UUID, treat as category ID
                    ...
                    // If it's a string, find category by name or slug
                    Optional<Product.Category> categoryDoc = categoryRepository.findByName(category);
                    if (categoryDoc.isPresent()) {
                        String categoryId = categoryDoc.get().getId();
                        filteredProducts = filteredProducts.stream()
                                .filter(product -> product.getCategory() != null && 
                                                 product.getCategory().getId().equals(categoryId))
                                .collect(Collectors.toList());
            ...
            if (gender != null && !gender.isEmpty()) {
                filteredProducts = filteredProducts.stream()
                        .filter(product -> product.getGender() != null && 
                                         product.getGender().equalsIgnoreCase(gender))
                        .collect(Collectors.toList());
            }
    ...
    public ResponseEntity<Map<String, Object>> createProduct(@RequestBody CreateProductRequest request, Authentication authentication) {
        try {
            ...
            if (request.getCategory() != null) {
                Optional<Product.Category> categoryOpt = categoryRepository.findBySlug(request.getCategory());
