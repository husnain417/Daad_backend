...
public class CategoryController {
    ...
    public ResponseEntity<Map<String, Object>> getAllCategories(
        ...
        // Stats (product counts by category)
        Map<String, Long> countsByCategory = productRepository.findAll().stream()
    ...
    public ResponseEntity<Map<String, Object>> addCategory(@RequestBody Map<String, Object> body) {
        ...
        Optional<Product.Category> existing = categoryRepository.findByName(name);
        ...
        String slug = name.toLowerCase().replaceAll("\\s+", "-").replaceAll("[^a-z0-9-]", "");
        ...
        if (parentCategory != null && !parentCategory.isEmpty()) {
            try {
                ...
                Optional<Product.Category> parent = categoryRepository.findById(parentIdStr);
                if (parent.isEmpty()) {
                    return ResponseEntity.status(400).body(Map.of("success", false, "message", "Parent category not found"));
                }
                parentId = parent.get().getId();
                calculatedLevel = (parent.get().getLevel() == null ? 0 : parent.get().getLevel()) + 1;
            } catch (Exception e) {
